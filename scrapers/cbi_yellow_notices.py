"""
CBI Interpol Yellow Notices scraper.

The page at https://cbi.gov.in/interpol-yellow-notice publishes a single
HTML table. Each row's link points at one of two destinations, which we
record verbatim in `link_kind`:

  - `interpol_notice` — link goes to interpol.int with a `#YYYY-NNNNN` notice
    fragment. We capture that fragment in `interpol_notice_id`.
  - `cbi_document`    — link goes to a CBI-hosted file under
    /assets/files/wanted/ (PDF or, in one case, JPG).
"""
...
LIST_URL = "https://cbi.gov.in/interpol-yellow-notice"
BASE_URL = "https://cbi.gov.in"

PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
OUTPUT_FILE = os.path.join(PROJECT_ROOT, "data", "cbi_yellow_notices.csv")
...
def parse_row(row, scraped_at):
    ...
    return {
        "source_agency": "CBI",
        "source_list": "Interpol Yellow Notice",
        "case_unit": "",
        "name": name,
        "father_name": "",
        "date_of_birth": "",
...
def run():
    print("=" * 60)
    print("CBI Interpol Yellow Notices scraper")
    print("=" * 60)
